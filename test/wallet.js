...
contract('Walley', (accounts) => {
  ...
  it('should have correct approvers and qorums', async () => {
  ...
  it('should NOT approve transfer if sender is not approved', async () => {
    await wallet.createTransfer(100, accounts[6], { from: accounts[0] });
    await expectRevert(wallet.approveTransfers(0, { from: accounts[6] }), 'Only appover allowed');
  });

  it('should NOT approve ransfer if transfer is already set', async () => {
    await wallet.createTransfer(100, accounts[6], { from: accounts[0] });
    await wallet.approveTransfers(0, { from: accounts[0] });
    await wallet.approveTransfers(0, { from: accounts[1] });
    await expectRevert(
